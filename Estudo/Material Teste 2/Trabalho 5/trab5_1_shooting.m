...
y(1) = 0;
v = nan(1,N);
v(1) = 0.2; %m/s valor à escolha, diferente de 0
...
subplot(1,2,1)
plot(x,y), xlabel('x'),ylabel('y'), title('W conhecido')
...
subplot(1,2,2)
plot(x,y), xlabel('x'),ylabel('y'), title('Valor de W aleatório')
...
% Podemos calcular o guess(3) e o result(3)
...
% While do Metódo de shooting
% Basta 2 guesses e 1 result para se poder saltar para o ciclo while
...
% i = 3;
% while abs(guess(i)-guess(i-1)) < 1E6 % O processo é repetido até a diferença dos guess não difira mais que uma determinada tolerância pré-estabelecida
%     for k = 1:N-1
