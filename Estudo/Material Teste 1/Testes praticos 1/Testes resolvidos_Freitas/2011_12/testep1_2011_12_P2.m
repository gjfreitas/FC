...
for k=1:N-1
    ...
    Cd = 0.2194+0.3262/(1+exp((norm(v(k,:))-dv)/5));
...
plot(r(:,1),r(:,2),'r.'), xlabel('x (m)'), ylabel('y (m)')
grid on
...
disp(['Alcance: ',num2str(alcance),' m'])
disp(['Tempo de voo: ',num2str(tempo_voo),' s'])
...
disp(['Velocidade Média: ',num2str(vmedia),' m/s'])
...
%% d) -> Calculando analiticamente a derivada
clc
clear all
...
%% d) -> Calculando numericamente a derivada
clc
clear all
