...
% fazemos (1:i-1) para fazer até ao ultimo positivo visto que o ultimo do
% vetor de z, na posicao i, é negativo

figure(1)
plot(x(1:i-1),z(1:i-1),'m'), xlabel('x (m)'), ylabel('altura (m)')
title('Trajetória do volante')

figure(2)
plot(t(1:i-1),z(1:i-1),'r'), xlabel('tempo (s)'), ylabel('altura (m)')
title('Evolução da altura do volante')


%interpolacao para descobrir o ponto em que a função z(t) é 0
t_impacto = interp1([z(i-1),z(i)],[t(i-1),t(i)],0);
disp(['Tempo de voo : ', num2str(t_impacto), ' segundos'])
x_impacto = interp1([t(i-1),t(i)],[x(i-1),x(i)],t_impacto);
...
[t_ode45,sol] = ode45(@f2,[t0 tf],[x0 vx0 z0 vz0], options);              %usa a funcao f em ficheiro separado
...
for k = 1:length(sol(:,3))
    if sol(k,3) >= 0 && sol(k+1,3) < 0
        ind = k;
    end
end
periodo = t_ode45(ind);
disp(['Tempo de voo : ', num2str(periodo), ' segundos'])
x_impacto = sol(ind,1);
disp(['alcance: ',num2str(x_impacto),' m'])
        
figure(2) % gráfico até z = 0
plot(sol(1:ind,1), sol(1:ind,3), '-m'), xlabel('Posição (m)'), ylabel('Altura (m)')
title('Volante de badminton - ode45')
